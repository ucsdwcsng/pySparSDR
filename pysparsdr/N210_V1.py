...
while True:
    #b = stdin.buffer.read(8) # read string then convert to binary number
    b = file.read(8)
    if (len(b) < 8):
        break
    ...
    if (is_avg):
        ...
        fixed_avg_time = ((time & (time_mask-1)) + avg_time_offset) * ts #calculate start time
